...
print(f"✅ Loaded data: {len(bnd_gdf)} polygons, {len(lineaments_gdf)} lineaments")
...
plt.show()  # Show fig_1 interactively
...
def run_iteration(
    ...
    def safe_intersection(row):
        try:
            ...
            if isinstance(geom, shapely.geometry.GeometryCollection):
                lines = [
                    g for g in geom.geoms if isinstance(g, shapely.geometry.LineString)
                ]
                if len(lines) == 0:
                    return None
                if len(lines) == 1:
    ...
    joined = joined[
        joined["clipped_geom"].apply(
            lambda x: isinstance(x, shapely.geometry.base.BaseGeometry)
        )
    ]
    joined["clipped_length"] = joined["clipped_geom"].apply(
...
def process_radius(radius):
    ...
    bnd_gdf["inner_polygon"] = bnd_gdf.geometry.buffer(buffer_distance)
    inner_poly = bnd_gdf["inner_polygon"].union_all()
    outer_poly = bnd_gdf.geometry.union_all()
...
plt.show()  # Show fig_2 interactively
plt.close()
...
print(f"\n⏱️ Total analysis time: {elapsed_seconds:.2f} seconds")
